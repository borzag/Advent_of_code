...
foreach (var line in File.ReadLines(@"Data\exampledata.txt"))
{
...
foreach (var directory in root.GetAllDirectories(root))
{
    ...
    if (directorySize <= sizeLimit)
        sizeSum += directorySize;
}

Console.WriteLine("\nTotal sum of file tree size,\n" +
                  $"where each node is of max size {sizeLimit}: {sizeSum}");

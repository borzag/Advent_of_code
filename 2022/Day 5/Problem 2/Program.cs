    ...
    private static void Main(string[] args)
    {
        ...
        foreach (string line in File.ReadLines(@"Data\inputdata.txt"))
        {
            ...
            else
            {
                ...
                { 
                    throw new Exception("Cant move crates from stack 0"); 
                }
                else
                {
                    var crates = lifoStacks[fromStack].Pop(itemCount);
                    lifoStacks[toStack].Add(crates);
                    nrOfMoves++;
                }
            }
        }
...
public class LifoStack
{
    ...
    public void Add(List<char> crates)
    {
        crates.Reverse();
    ...
    public List<char> Pop(int nrOfCrates)
    {
        var result = new List<char>();
        for (int i = 1; i <= nrOfCrates; i++)
        { 
            result.Add(lifo.Pop()); 
        }
        return result;
